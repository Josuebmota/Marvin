...
1. **Ler a fonte antes de escrever o alcance.** Leia o README/docs do que está instalado
   (`~/.Codex/plugins/cache/<nome>/...`) — não a memória. Na 11b o alcance do Cursor saiu
...
2. **Uma entrada em `FERR_OPCIONAIS`:** `nome`, `detecta()` (devolve `false` ou o estado
   ...
   Codex lê `~/.Codex/...` via `os.homedir()` — é o que o teste redireciona.
...
- Alcance por plataforma que a detecção não cobre: **declare** ("só o do Codex é
